...
# def encode_single_method(ts_path, output_dir, encoding_method, image_size=224):
#     """
#     Apply a single encoding method to a time series file
    
#     Args:
#         ts_path (str): Path to time series data file (.npy)
#         output_dir (str): Directory to save encoded images
#         encoding_method (str): One of 'gasf', 'gadf', 'mtf', 'rp'
#         image_size (int): Size of the output images
...
#     if encoding_method == 'gasf':
#         img = create_gasf(ts, image_size)
#         suffix = '_gasf.png'
#     elif encoding_method == 'gadf':
#         img = create_gadf(ts, image_size)
#         suffix = '_gadf.png'
#     elif encoding_method == 'mtf':
#         img = create_mtf(ts, image_size)
#         suffix = '_mtf.png'
#     elif encoding_method == 'rp':
#         img = create_rp(ts, image_size)
#         suffix = '_rp.png'
#     else:
#         raise ValueError(f"Unknown encoding method: {encoding_method}")
    
#     # Save image
#     img_path = os.path.join(output_dir, f"{base_filename}{suffix}")
...
# def batch_encode_time_series(input_dir, output_dir, image_size=224, encoding_method=None):
#     """
#     Batch process all time series files in a directory
...
#         output_dir (str): Directory to save encoded images
#         image_size (int): Size of the output images
#         encoding_method (str, optional): Specific encoding method ('gasf', 'gadf', 'mtf', 'rp')
#                                         If None, all methods are applied
#     """
#     # Get all .npy files
#     ts_files = [os.path.join(input_dir, f) for f in os.listdir(input_dir) if f.endswith('.npy')]
...
#     if encoding_method:
#         # Process with single encoding method
#         desc = f"Encoding time series ({encoding_method.upper()})"
#         for ts_path in tqdm(ts_files, desc=desc):
#             encode_single_method(ts_path, output_dir, encoding_method, image_size)
#     else:
#         # Process with all encoding methods
#         desc = "Encoding time series (all methods)"
#         for ts_path in tqdm(ts_files, desc=desc):
#             encode_all_methods(ts_path, output_dir, image_size)
...
def normalize_ts(ts, min_val=-1, max_val=1, method='minmax'):
    ...
    if method == 'minmax':
        ...
        if ts_std == 0:
            return np.zeros_like(ts)
        normalized = (ts - ts_mean) / ts_std
        # Scale to desired range
        norm_min, norm_max = np.min(normalized), np.max(normalized)
        if norm_min == norm_max:
            return np.zeros_like(ts)
        return min_val + ((normalized - norm_min) * (max_val - min_val)) / (norm_max - norm_min)
...
def create_mtf(ts, image_size=224, n_bins=8):  # Reduced bins for audio data
    """Enhanced MTF with better binning for audio features"""
    
    # Use z-score normalization for MTF
    normalized_ts = normalize_ts(ts, method='zscore')
    
    if len(normalized_ts) != image_size:
        indices = np.linspace(0, len(normalized_ts) - 1, image_size).astype(int)
        normalized_ts = normalized_ts[indices]
    
    # Use percentile-based binning instead of linear
    bins = np.percentile(normalized_ts, np.linspace(0, 100, n_bins + 1))
    bins = np.unique(bins)  # Remove duplicates
    if len(bins) < 2:
        bins = np.linspace(np.min(normalized_ts), np.max(normalized_ts), n_bins + 1)
    
    binned_ts = np.digitize(normalized_ts, bins) - 1
    binned_ts = np.clip(binned_ts, 0, len(bins) - 2)
